...
- [x] **7. Убрать runtime-загрузку Vosk-модели — поставлять модель внутри APK**
  (исходная формулировка аудитов: Zip Slip и отсутствие проверки целостности при
  загрузке модели — проблема снята целиком, т.к. загрузки в рантайме больше нет).
  Решение: модель `vosk-model-small-ru-0.22` входит в APK как распакованные
  `assets/model-ru/` (по образцу официального Vosk Android demo). Версия, URL и
  SHA-256 архива зафиксированы в `app/build.gradle.kts` (задачи `downloadVoskModel`/
  `unpackVoskModel` скачивают и распаковывают модель при сборке). `ModelManager`
  при первом запуске атомарно копирует модель из assets во внутреннее хранилище
  через staging-каталог: готовая модель видна приложению только после полной
  установки (маркер `.ready` с версией пишется последним), при ошибке staging
  удаляется целиком, прерванная установка повторяется корректно, устаревшая
  версия маркера вызывает переустановку. Приложение не обращается к сети вообще;
  разрешение INTERNET удалено из манифеста. Attribution Apache 2.0 —
  `app/src/main/assets/vosk-model-NOTICE.txt` (входит в APK). Покрыто unit-тестами
  `ModelManagerTest` (7 тестов).
  (devin P2, gpt-5 #2, opus #6)

- [x] **8. Ручная правка очков тихо засчитывает гейм**
...
- [x] **9. Ручная правка геймов даёт ложный звуковой сигнал и TTS-объявление**
  Переход отслеживался по росту `totalGames` без учёта источника изменения — правка
  геймов (например 2:1 → 4:1) воспроизводила звонок и объявляла «Гейм, Игрок 1»,
...
- [x] **10. Полный лог матча пишется в SharedPreferences на каждое событие**
  ...
  Кроме того, `persistCurrentMatch()` дедуплицирует записи по сериализованному
  состоянию (`lastPersistedState`): на log-only/speech-событиях (нераспознанная
  фраза, «сколько», отклонённая команда) одинаковый `MatchState` не перезаписывает
  файл настроек — запись идёт только при реальном изменении счёта.
...
- [x] **17. Keep-alive аудиотракта держится весь матч — не баг, а фича**
  Изначально предполагалось, что непрерывный поток тишины на `USAGE_MEDIA` держит
  A2DP-линк и аудиотракт активными все 1.5–2 часа матча, давая лишний расход
  батареи на телефоне и наушниках.
  Проверено: `SignalPlayer.setKeepAlive` включается/выключается строго вместе с
  режимом «Слушать» (`MatchViewModel.startListening`/`stopListening`), а не на
  весь матч — при паузе или остановке прослушивания поток тишины сразу
  останавливается. Сам A2DP-линк код не контролирует; пока в наушниках и так
  играет музыка (типичный сценарий использования — распознавание счёта включено
  почти весь матч, музыка тоже играет постоянно), A2DP-линк и аудио-HAL уже
  подняты независимо от приложения, и доп. расход батареи от собственного
  потока тишины поверх этого минимален. Оставлять как есть — принято решение
  не менять текущее поведение.
  Небольшая доработка всё же сделана: на реальной паузе матча (`togglePause`)
